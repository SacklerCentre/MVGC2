...
function [A,V,E] = tsdata_to_var1(X,p,regmode,dm)
...
if  strcmpi(regmode,'OLS') % OLS (QR decomposition)
	...
	XX = zeros(n,q,m+q,N);
	for k = 1:q
		XX(:,k,k+1:k+m,:) = X; % k-lagged observations
	end
...
elseif strcmpi(regmode,'LWR') % LWR (Morf)
    ...
    if nargout > 1
        ...
        V = (E*E')/(M-1);                                       % residuals covariance matrix
        E = reshape(E,n,m-k,N);                                 % put residuals back into per-trial form
    end
