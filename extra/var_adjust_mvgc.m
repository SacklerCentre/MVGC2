...
% be tested; that is, check that iters <= maxi and that abs(F-FF)
% is tolerably small (or zero).

assert(F >= 0,'Target GC must be non-negative!');

if nargin < 6 || isempty(maxi), maxi = 500; end
...
while true
	...
	AA = specnorm(AA,rho);
	FF = var_to_mvgc(AA,V,x,y);
	if FF > F+eps, fmax = f; elseif FF < F-eps, fmin = f; else, return; end % new box
	if iters > maxi, return; end
end
