function [Y,X,mtrunc] = varfima_to_tsdata1(A,B,fiparms,X,m,N,mtrunc,decfac)
...
if N > 1 % multi-trial
    Y = zeros(n,m,N);
  	if have_var
		if have_vma
			if fint
				for k = 1:N
					Y(:,:,k) = genvar(A,genvma(B,gendiff(X(:,:,k),fiparms)));
				end
			else
				for k = 1:N
			...
			if fint
				for k = 1:N
					Y(:,:,k) = genvar(A,gendiff(X(:,:,k),fiparms));
				end
			else
				for k = 1:N
					Y(:,:,k) = genvar(A,X(:,:,k));
		...
		if have_vma
			if fint
				for k = 1:N
					Y(:,:,k) = genvma(B,gendiff(X(:,:,k),fiparms));
				end
			else
				for k = 1:N
					Y(:,:,k) = genvma(B,X(:,:,k));
	...
	if have_var
		if have_vma
			if fint
				Y = genvar(A,genvma(B,gendiff(X,fiparms)));
			else
				Y = genvar(A,genvma(B,X));
			end
		else
			if fint
				Y = genvar(A,gendiff(X,fiparms));
			else
				Y = genvar(A,X);
			end
		end
