...
% with coefficients matrix |A| and residuals covariance matrix |V|. |fres|
...
function [S,H] = var_to_cpsd(A,V,fres,autospec)
...
if autospec
	assert(nargout < 2,'Transfer function is not calculated for autospectra-only option');
	S = zeros(h,n);
	for k = 1:h
		HVLk = AF(:,:,k)\VL;
		Sk = HVLk*HVLk';
